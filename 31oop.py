...
# 벗어나 독립된 객체들의 모음이라고 보는 시각에 근거 해서
# 프로그래밍 하는 패러다임
...
# OOP에서 클래스의 종류
# 1. 값만 저장하는 클래스        : VO;Value-Object, DTO
...
# 클래스 내에 선언된 맴버 변수 - 맴버변수 (주로 private 접근제한자 사용)

class SungJuk:
    # 생성자 : 클래스를 이용해서 객체를 만들때 초기화 작업을 수행하는 특별한 함수
    # 변수선언 : self.변수명
    def __init__(self):
        self.name = '홍길동'
        self.kor = 0
...
# 객체의 맴버변수 접근 : 객체명.맴버변수명
print(emp.empid)
print(emp.fname, emp.lname, emp.email,emp.phone,emp.hdate,
...
class SungJuk2:
    def __init__(self, name,kor,eng,mat):   # 초기화 할것들
        self.name = name                    # 밑에 넣기
        self.kor = kor                      # 밑에 넣기
        self.eng = eng                      # 밑에 넣기
        self.mat = mat                      # 밑에 넣기
        self.tot = 0
        self.avg = 0.0
        self.grd = '가'
    # __str__ : 맴버 변수들의 값을 문자열화 해서
    # 객체 정보를 외부에 출력할때 사용하는 특수한 함수
    def __str__(self):
