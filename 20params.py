...
uid, pwd = member.keys()
print(uid, pwd)
uid, pwd = member.items() #py 3.8이상만 된다
print(uid, pwd)
...
def squr(x):
    # 출력까지 함수에서 구현하기 떄문에
    # 플랫폼이 변하는 경우 함수를 재작성해야하는 번거로움이 발생!
    print(f'{x} => {x ** 2}')
...
# 즉, return 문 이후의 코드는 실행하지않음- 함수 조지 종료 가능
